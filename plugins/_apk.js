...
if (!text) throw `*تحميل التطبيقات والالعاب مع لقمان بوت*`    
let searchA = await search(text)
if (searchA.length < 1) return await  conn.sendMessage(m.chat, { text:'  *لم يتم العثور على اي تطبيق بهذا الاسم*'  }, { quoted: m } )
var srh = [];  
for (var i = 0; i < searchA.length; i++) {
srh.push({ title: searchA[i].name, description: ''  , rowId: prefix +  'micha'   + searchA[i].id });}
...
break
case  'apk' : case  'aptoidedl' :
if (!text) throw `*تحميل التطبيقات والالعاب: ${prefix + command}* https://rexdl.com/android/minecraft-pocket-edition-apk-download1.html/`  
let data5 = await download(text)
if ( data5.size.replace(  'MB'  , ''  ) > 200) return await conn.sendMessage(m.chat, { text: ' *♥'  }, { quoted: m } )
...
throw `*تحميل التطبيقات والالعاب:*\n ${prefix + command} facebook  lite`
}}    
handler.command = /^(apkmoad|modaapk|dapk2|apk|aptoidedl)$/i
